...
const BUILD_TAG = "2026-06-28.275";  // bump each deploy so prod vs source is unambiguous
...
app.get("/api/phoenix/seed", async (c) => {
  try {
    ...
    try {
      ...
      const { ensureLaunchpadSchema, seedLaunchpadIdeas } = await import("./ensure-launchpad-schema");
      await ensureLaunchpadSchema();
      await seedLaunchpadIdeas();
      ...
      const { ensureCashBookSchema } = await import("./ensure-cash-book-schema");
      await ensureCashBookSchema();
      const { ensureBackupSchema } = await import("./ensure-backup-schema");
      await ensureBackupSchema();
      const { ensureOpportunitiesSchema } = await import("./ensure-opportunities-schema");
      await ensureOpportunitiesSchema();
      // Daily automatic data backup (once per UTC day) — keeps the live client list +
      // every business table snapshotted so nothing's lost. Non-blocking, best-effort.
      const { autoSnapshotIfDue } = await import("./backup-router");
...
async function startServer() {
  ...
  try {
    ...
    const { ensureFaxSchema } = await import("./ensure-fax-schema");
    ...
    const { ensureYearEndSchema } = await import("./ensure-year-end-schema");
    await ensureYearEndSchema();
    const { seedSelectivePaintingYearEnd } = await import("./seed-selective-yearend");
    await seedSelectivePaintingYearEnd();
  ...
  if (process.env.FIGGY_SKIP_CLIENT_SEED !== "on") {
    ...
    try {
      const { backfillSetupTasks, pruneCraRacTasksForDone } = await import("./task-generator");
      const s = await backfillSetupTasks();
      console.log(`[setup-tasks] ensured for ${s.clients} clients, +${s.created} created`);
      // Clear stale CRA RAC setup tasks for clients whose intake marks it done.
      await pruneCraRacTasksForDone();
    } catch (e) {
