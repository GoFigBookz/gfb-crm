...
const BUILD_TAG = "2026-06-26.154";  // bump each deploy so prod vs source is unambiguous
...
app.get("/api/phoenix/seed", async (c) => {
  try {
    ...
    try {
      ...
      const { seedBrain, seedAgentBrain, seedKnowledgeBrain, seedConstitution } = await import("./brain-store");
      await seedBrain();
      await seedAgentBrain();
      await seedKnowledgeBrain();
      await seedConstitution();
      const { seedHeritage } = await import("./seed-heritage");
      await seedHeritage();
