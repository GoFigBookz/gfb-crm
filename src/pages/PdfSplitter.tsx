...
export default function PdfSplitter() {
  ...
  const reset = () => { setFile(null); setBuffer(null); setPageCount(0); setDocs(null); setError(null); };
  ...
  const processFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      const r = await plan.mutateAsync({ base64: abToBase64(buf), fileName: f.name, pageCount: total });
      if (!r.ok) { setError(r.error); return; }
      if (!r.documents.length) { setError("No documents detected. Try a clearer scan."); return; }
      // Clamp ranges to the real page count.
      setDocs(r.documents.map((d) => ({ ...d, startPage: Math.min(Math.max(1, d.startPage), total), endPage: Math.min(Math.max(1, d.endPage), total) })));
    } catch (e: any) {
      ...
      {!docs && (
        <div
          ...
          className={cn("border-2 border-dashed rounded-xl p-12 text-center transition-all",
            plan.isPending ? "border-lime-400 bg-lime-50/60" : isDragging ? "border-lime-500 bg-lime-50" : "border-slate-300 bg-slate-50")}
        >
          {plan.isPending ? (
            <>
              <div className="h-12 w-12 mx-auto mb-4 rounded-full border-4 border-lime-200 border-t-lime-500 animate-spin" />
              <p className="text-lg font-medium text-slate-700 mb-2">Reading your scan…</p>
              <p className="text-sm text-slate-500">Figgy is finding where each document starts and ends. A big batch can take up to a minute.</p>
            </>
          ) : (
            <>
              ...
              <p className="text-sm text-slate-500 mb-4">One PDF containing several documents (statements, invoices, receipts, asset docs…). Up to 100 pages.</p>
