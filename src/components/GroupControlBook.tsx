...
import { BookOpen, ShieldCheck, Coins, Users, Building2 } from "lucide-react";
 ...
 * CRM — entities, cap table, a dividend report with per-person attribution, the
 * family-benefit tracker, and an Individuals-with-Significant-Control register. WIP.
 */
          ...
          <div className="flex gap-1">
            {data.fiscalYears.map((y: string) => (
              <button key={y} onClick={() => setFy(y)}
                className={`px-2 py-0.5 rounded text-xs border ${data.fiscalYear === y ? "bg-indigo-500 text-white border-indigo-500" : "bg-white border-slate-200 hover:bg-slate-50"}`}>FY{y}</button>
            ))}
          </div>
        }>
