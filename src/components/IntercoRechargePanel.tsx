...
export default function IntercoRechargePanel({ defaultPayerId }: { defaultPayerId?: number } = {}) {
  ...
  const billableRecon = trpc.intercoRecharge.billableReconcile.useMutation();
  ...
  const postLive = () => {
    ...
    if (tie && !tie.error && !tie.ties) {
      warn = `\n\nNote: this period charges ${money(tie.rechargeHst)} HST; the HST account holds ${money(tie.target)} (difference ${money(tie.variance)} = prior-period exceptions you're handling manually). This Mar-to-date recharge is correct — after you also add the manual exception(s), the HST account clears.\n\nPost this period now?`;
    }
    ...
    <Card className="border-violet-200">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button size="sm" variant="outline" disabled={billableRecon.isPending || !payerId}
            onClick={() => payerId && billableRecon.mutate({ payerClientId: payerId, from: start, to: end })}>
            {billableRecon.isPending ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : null} Reconcile this period vs HST
          </Button>
          {!chargeHst && <span className="text-xs text-amber-600">No HST (Section 156 election).</span>}
        </div>

        {billableRecon.data && !billableRecon.data.ok && (
          <div className="text-xs text-amber-700">
            {billableRecon.data.error === "bridge_not_returning_data" ? "The live QBO connection isn't returning data yet (bridge config — not the books)." : `Couldn't read the books (${billableRecon.data.error}).`}
          </div>
        )}
        {billableRecon.data && billableRecon.data.ok && (
          <div className={`rounded-lg border p-2 space-y-1 text-xs ${billableRecon.data.ties ? "border-emerald-300 bg-emerald-50" : "border-amber-300 bg-amber-50"}`}>
            <div className="font-medium text-slate-700">Fiscal-YTD expenses {billableRecon.data.from} → {billableRecon.data.to}: <b>{money(billableRecon.data.totalExpenses)}</b> across {billableRecon.data.txnCount} bills/expenses{billableRecon.data.itemLines > 0 ? ` (incl. ${billableRecon.data.itemLines} item lines)` : ""}.</div>
            <div className={billableRecon.data.ties ? "text-emerald-800" : "text-amber-800"}>
              HST on those expenses <b>{money(billableRecon.data.totalHst)}</b> vs HST account <b>{money(billableRecon.data.hstAccountBalance)}</b> → difference <b>{money(billableRecon.data.variance)}</b> (prior-period exceptions, handled manually).
            </div>
            ...
            <div className="text-[11px] text-slate-400">HST account: {billableRecon.data.hstAccounts.map((a: any) => `${a.name} ${money(a.balance)}`).join(" · ")}.</div>
          </div>
        )}
            ...
            {(r as any)?.hstTie && !(r as any).hstTie.error && (
              (r as any).hstTie.ties ? (
                ...
                <div className="rounded-md border border-amber-300 bg-amber-50 p-2 text-xs text-amber-900 space-y-1">
                  <div className="font-semibold flex items-center gap-1"><AlertTriangle className="h-3.5 w-3.5" /> This period charges {money((r as any).hstTie.rechargeHst)} HST · the HST account holds {money((r as any).hstTie.target)} · difference {money((r as any).hstTie.variance)}.</div>
                  <div>The difference (~<b>{money((r as any).hstTie.impliedMissingBase)}</b> of expenses) is <b>prior-period exceptions</b> dated before this window — handle those <b>manually</b> (e.g. the New Air Mechanical bill). This period's Mar-to-date recharge is correct on its own; once you post it and add the manual exception(s), the HST account clears to $0.</div>
                  {Array.isArray((r as any).hstTie.accounts) && (r as any).hstTie.accounts.length > 0 && (
                    <div className="text-amber-700">HST account(s): {(r as any).hstTie.accounts.map((a: any) => `${a.name} ${money(a.balance)}`).join(" · ")}</div>
                  )}
                </div>
              )
            )}
